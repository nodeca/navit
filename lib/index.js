...
Navit.prototype.__initPhantom__ = function (callback) {
  ...
  switch (this.__options__.engine) {
    case 'phantomjs':
      ...
      driverOptions.phantomPath = require('phantomjs').path;
      break;
    case 'slimerjs':
      driver = require('node-slimerjs');

      // Standalone SlimerJS not required because `node-slimerjs` already contains old version of SlimerJS
      try {
        driverOptions.slimerPath = require('slimerjs').path;
      } catch (__) {}
      break;
  }

  driver.create(function (err, instance) {
    if (err) {
      callback(err);
      return;
    }

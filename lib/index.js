...
var phDriver   = require('node-phantom-simple');
var elDriver   = require('./electron/driver');
...
function Navit(options, engineOptions) {
  ...
  // Register functions
  var functions = this.__options__.engine === 'electron' ? elFunctions : phFunctions;
...
Navit.prototype.run = function (teardown, callback) {
  ...
  async.series([
    ...
    function (next) {
      ...
      if (self.__options__.engine === 'electron') {
        self.__initElPage__(next);
      } else {
        self.__initPhPage__(next);
      }
    },
...
Navit.prototype.__initPhantom__ = function (callback) {
  ...
  currentDriver.create(driverOptions, function (err, instance) {
    if (err) {
      callback(err);
      return;
    }
...
Navit.prototype.__initElPage__ = function (callback) {
  var self = this;
  ...
  self.__engine__.createPage(function (err, page) {
    ...
    async.series([
      function (next) {
        page.exec('setSize', [ 1600, 900, false ], next);
      },
...
Navit.prototype.__initPhPage__ = function (callback) {
  var self = this;

  ...
  function get_cookies(fn) {
    this.__queue__.push(function get_cookies_step(callback) {
      ...
      this.__page__.invoke('win', 'webContents.getURL', (err, url) => {
        ...
        if (!url) {
          callback(new NavitError('You should open the page before try to get cookies'));
          return;
        }

        let filter = { url };

        this.__page__.invoke('winAsync', 'webContents.session.cookies.get', filter, (err, args) => {
          if (err) return callback(err);

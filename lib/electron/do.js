...
async function open(url, options) {
  ...
  for (let scriptPath of this.__options__.inject) {
    let fileContent = readFileSync(scriptPath, 'utf-8');
...
functions.push([
  ...
  function do_open(url, options) {
    this.__queue__.push(async function do_open_step() {
      ...
      let opt = Object.assign({}, { method: 'GET', data: {}, headers: {} }, options);

      opt.headers = Object.assign({}, this.__headers__, opt.headers);

      await open.call(this, link, opt);
    });

    if (isFunction(this.afterOpen)) this.afterOpen();
  ...
  function do_post(url, data, options) {
    this.__queue__.push(async function do_post_step() {
      ...
      let opt = Object.assign(
        {},
        { method: 'POST', data: Buffer.from(data), headers: {} },
        options
      );
...
functions.push([
  ...
  function screenshot(/*selector|boundingRect, type, path*/) {
    ...
    if (arguments.length > 1) {
      if (typeof arguments[0] === 'string' || isFunction(arguments[0])) {
    ...
    this.__queue__.push(async function screenshot_step() {
      ...
      if (boundingRect) {
        let newRect = Array.isArray(boundingRect) ? {
          x: boundingRect[0],
          y: boundingRect[1],

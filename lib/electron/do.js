  ...
  function do_reload() {
    this.__queue__.push(function do_reload_step(callback) {
      ...
      async.series([
        ...
        next => this.__page__.exec('webContents.reload', [], next),
...
functions.push([
  ...
  function do_upload(selector, path) {
    this.__queue__.push(function do_upload_step(callback) {
      ...
      this.__page__.execFnAsync(function (selector, filePath, cb) {
...
functions.push([
  ...
  function screenshot(/*selector|boundingRect, type, path*/) {
    ...
    let saveImage = function (opts, cb) {
      ...
      // renderer wait.
      setTimeout(() => {
        this.__page__.execFnAsync(capture, [ opts ], (err, result) => {

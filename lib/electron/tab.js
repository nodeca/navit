...
functions.push([
  ...
  function tab_switch(index) {
    ...
    this.__queue__.push(function tab_switch_step(callback) {
      ...
      debug(format('tab.switch(%d)', i));

      if (i < 0) {
        i = self.__tabs__.length + i;
      }
...
functions.push([
  ...
  function tab_close(index) {
    ...
    this.__queue__.push(function tab_close_step(callback) {
      ...
      if (i === self.__tabIndex__ && self.__tabs__.length !== 1) {
        var nextTabIndex;
      ...
      self.__tabs__[i].page.close(function (err) {
        ...
        self.__tabIndex__ = _.findIndex(self.__tabs__, function (tab) {
          return tab.page === self.__page__;
        });

        // If all tabs closed - open one new tab
        if (self.__tabIndex__ === -1) {
          self.__initPhPage__(callback);
          return;
        }

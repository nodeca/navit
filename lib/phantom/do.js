...
function open(url, options, callback) {
  var self = this;
  var oldHeaders, pageSettings, realUrl;

  async.series([

    // Get request headers
    function (next) {
      self.__page__.get('customHeaders', function (err, headers) {
        if (err) {
          next(err);
          return;
        }
    ...
    function (next) {
      self.__page__.clearMemoryCache(next);
    },

    // Set custom request headers
    function (next) {
      self.__page__.set('customHeaders', _.assign({}, oldHeaders, options.headers), next);
    },

    // Get page settings
    function (next) {
      self.__page__.get('settings', function (err, settings) {
        if (err) {
          next(err);
          return;
        }
    ...
    // Wait loading
    function (next) {
      self.__pageWait__(null, null, next);
    },

    // Restore old headers after request
    function (next) {
      self.__page__.set('customHeaders', oldHeaders, next);
    },

    // Get real url (may change after redirect)
    function (next) {
      self.__page__.get('url', function (err, url) {
        if (err) {
          next(err);
          return;
        }
...
functions.push([
  ...
  function do_reload() {
    ...
    self.__queue__.push(function do_reload_step(callback) {
      debug('do.reload()');

      self.__page__.evaluate(function () {
        ...
        self.__pageWait__(null, null, function (err) {
          if (err) {
            callback(err);
            return;
          }
  ...
  function do_select(selector, option) {
    this.__queue__.push(function do_select_step(callback) {
      ...
      this.__page__.evaluate(function (selector, option) {
        ...
        return true;
      }, sel, _.isFunction(option) ? option() : option, function (err, exists) {
        if (err) {
          callback(err);
          return;
        }
